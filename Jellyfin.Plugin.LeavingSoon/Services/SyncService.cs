...
public class SyncService : IScheduledTask
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // A library cover is a dynamic collage only generated when the CollectionFolder's
        // own metadata is refreshed after items-with-images exist. Neither a scoped path
        // refresh nor Jellyfin's first-scan ordering does that, so a fresh library shows no
        // cover until a full scan. Refresh the cover explicitly once the items are indexed.
        //
        // Force the refresh only when this sync is re-enabling a library that was disabled
        // while empty (not on first creation): the collage of the previous leaving set would
        // otherwise linger (Jellyfin only regenerates an existing cover when the refresh
        // explicitly replaces it), which is the wrong cover for a churn-driven leaving-soon
        // library. First creation has no stale cover, so the normal gate (null image →
        // regenerate) and the lighter refresh path apply there.
        //
        // The wait is bounded and shorter on the initial-scan path: that path queues a
        // global scan that usually outlasts any wait we can afford, so a cover generated by
        // the next sync is fine and the sync (and the debug endpoint behind the lock)
        // shouldn't stall for it.
        var forceRefresh = !needsInitialScan && !_virtualFolderManager.IsLibraryEnabled(libraryName);
        var timeout = needsInitialScan ? TimeSpan.FromSeconds(10) : TimeSpan.FromSeconds(30);
